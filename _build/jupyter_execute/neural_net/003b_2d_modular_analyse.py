...
Preloaded models from 002b_1d_modular_fit.ipynb
Model are fitted to previously spli training and test data sets.

# Aims

Test performance of model using subnets for:

1. *Patient clinical data*: Age, gender, ethnicity, disability before stroke, stroke scale data. Pass through one hidden layer (with 2x neurons as input features) and then to two neurones with sigmoid activation.

2. *Pathway process data*: Times of arrival and scan, time of day, day of week. Pass through one hidden layer (with 2x neurons as input features) and then to two neurones with sigmoid activation.

3. *Hospital ID* (one-hot encoded): Connect input directly to two neurones with sigmoid activation.
...
plt.savefig('./output/nn_2d_roc_sens_spec.jpg', dpi=300)
...
# Save
filename = './predictions/nn_2d_k_fold.csv'
...
plt.tight_layout(pad=2)
plt.savefig('./output/nn_2d_reliability.jpg', dpi=300)
plt.show()
...
* Overall accuracy = 85.2% (90.1% for those 66% samples with at least 80% confidence of model)
* Using nominal threshold (50% probability), specificity (89%) is greater than sensitivity (75%)
* The model can achieve 84.2% sensitivity and specificity simultaneously
* ROC AUC = 0.919