...
The three subnet outputs are then past to a single neurone with sigmoid activation for final output.

Note: In this experiment the output of each subnet is a vector with a single value (a 1D output), but the model is structured to allow for more neurons(dimensions) for each subnet, e.g. to allow for encoding of hospitals that allow more coplex interactions with patients (which may also have >1 dimension output).
...
plt.savefig('./output/nn_1d_roc_sens_spec.jpg', dpi=300)
...
# Save
filename = './predictions/nn_1d_k_fold.csv'
...
plt.tight_layout(pad=2)
plt.savefig('./output/nn_1d_reliability.jpg', dpi=300)
plt.show()
...
* Overall accuracy = 85.5% (90.5% for those 65% samples with at least 80% confidence of model)
* Using nominal threshold (50% probability), specificity (89%) is greater than sensitivity (76%)
* The model can achieve 84.5% sensitivity and specificity simultaneously
* ROC AUC = 0.921